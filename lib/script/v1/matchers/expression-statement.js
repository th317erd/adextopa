...
export class ExpressionStatementToken extends Token {
  static [Utils.TYPE_SYMBOL] = 'ExpressionStatementToken';
...
export const ExpressionStatement = Matcher.createMatcherMethod(({ name, literals, functionCalls, identifiers, memberExpression }) => {
  return MapResult(
    Program(
      ...
      // (memberExpression !== false) && MemberExpression,
    ...
    ({ context }, result) => {
      ...
      let expressionToken = new ExpressionStatementToken({ context, token });
