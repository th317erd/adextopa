...
export const ProcessMatcher = Utils.makeKeysNonEnumerable(class ProcessMatcher extends Matcher {
  ...
  async exec(context) {
    ...
    context.debugLog(`${context.debugColor('fg:cyan', matcherName)}: Starting iteration ${context.debugColor('bg:cyan')}@${start}-${context.range.end}${context.debugColor('bg:black')} [${this.debugPosition(context)}]: { startIndex: ${startIndex}, endIndex: ${endIndex} }`);
