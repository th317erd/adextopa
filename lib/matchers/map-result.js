...
export const MapMatcher = Utils.makeKeysNonEnumerable(class MapMatcher extends Matcher {
  ...
  createMatcherScope(context) {
    ...
    return {
      ...
      tokensOnly: context.resolveValueToBoolean(this._tokensOnly),
    };
  }
...
export const MapResult = Matcher.createMatcherMethod((matcher, mapMethod) => {
...
export const MapToken = Matcher.createMatcherMethod((matcher, mapMethod) => {
  return new MapMatcher({
    matcher:    stringToFetch(matcher),
    mapMethod:  mapMethod,
    tokensOnly: true,
