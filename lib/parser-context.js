...
export class ParserContext {
  ...
  constructor(parser, _opts) {
    ...
    if (typeof opts.debug === 'boolean')
      this.assignToScope({ 'debug': opts.debug });

    if (typeof opts.debugColors === 'boolean')
      this.assignToScope({ 'debugColors': opts.debugColors });
  }
  ...
  fetchProp(context, keyName, opts) {
    ...
    else if (this.scope.has(keyName))
      return this.scope.get(keyName);

    return (opts && opts.defaultValue);
  }

  getFromScope(keyName) {
    return this.scope.get(keyName);
  }
  ...
  setDebugMode(value) {
    this.assignToScope({ debug: !!value });
    return this;
  }

  setDebugColors(value) {
    this.assignToScope({ debugColors: !!value });
    return this;
  }
  ...
  debugLog(...args) {
    if (this.getFromScope('debug') !== true)
      return;
  ...
  [MatcherResult.RESULT_SEEK](result) {
    return result;
  }
