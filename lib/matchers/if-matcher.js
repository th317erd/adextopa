...
export const IfMatcher = Utils.makeKeysNonEnumerable(class IfMatcher extends SwitchMatcher {
  ...
  createMatcherScope(context) {
    ...
    return {
      ...matcherScope,
      action:     context.resolveValueToMatcherOrMethod(this.action),
      elseAction: context.resolveValueToMatcherOrMethod(this.elseAction),
      invert:     context.resolveValueToBoolean(this.invert),
    };
  }
...
export const IfAll = Matcher.createMatcherMethod((_, ...matchers) => {
...
export const IfAny = Matcher.createMatcherMethod((_, ...matchers) => {
  return new IfMatcher({
    matchers:   matchers.map(stringToFetch).filter(Boolean),
    matchAll:   false,
  });
...
export const IfNotAll = Matcher.createMatcherMethod((_, ...matchers) => {
  return IfAll(...matchers).Invert(true).name('IfNotAll');
});

export const IfNot = IfNotAll;

export const IfNotAny = Matcher.createMatcherMethod((_, ...matchers) => {
