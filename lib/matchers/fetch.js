...
export class FetchMatcher extends Matcher {
  ...
  [Matcher.VIRTUAL_RESOLVER](context, opts) {
    let valueResolver = new ValueResolver(context, 'Fetch', opts);
    return valueResolver.fetchProp(context, this.keyName, { defaultValue: this.defaultValue });
  }

  async exec(context) {
    let value = this.resolveValue(context, this, { skipDefaultFetch: true });
