  ...
  async exec(matcherScope) {
    ...
    if (token) {
      ...
      if (this.updateCapturedRange) {
        ...
        return context.updateParserOffsetResult(newOffset, matcherResult).token(null).parserRange(null);
      } else {
        context.debugLog(`${context.debugColor('fg:cyan', matcherName)}: Discarding resulting token (and not updating captured range).`);
        return context.updateParserOffsetResult(newOffset, matcherResult).token(null).capturedRange(null).parserRange(null);
      }
    }
