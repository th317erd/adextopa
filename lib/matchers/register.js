...
export class RegisterMatcher extends Matcher {
  ...
  async exec(context) {
    ...
    if (!keyName)
      return this.errorResult(context, 'Register: Specified key to fetch from scope is empty.');

    let scope = context.scope;
    let value = this.resolveValue(context, this.value);

    if (!(value instanceof Matcher))
      return this.errorResult(context, 'Register: Provided value is not a matcher.');
