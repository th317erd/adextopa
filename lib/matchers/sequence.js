...
export class SequenceMatcher extends Matcher {
  ...
  constructor(_opts) {
    let opts          = _opts || {};
    let startPattern  = opts.startPattern;
    let endPattern    = opts.endPattern;
    let escapePattern = opts.escapePattern;
    let abortPatterns = opts.abortPatterns;
  ...
  exec(context) {
    const resolveArgument = (name, error) => {
      let value = this.resolveValue(context, this[name], { wantPrimitiveValue: true });
      if (value instanceof String)
        value = value.valueOf();

      if (error && (typeof value !== 'string' && !(value instanceof RegExp)))
        return this.panicResult(context, error);
    ...
    let startPattern  = resolveArgument('startPattern', `${this.debugColor(context, 'fg:cyan', matcherName)}: "startPattern" must be a string or a RegExp.`);
    let endPattern    = resolveArgument('endPattern', `${this.debugColor(context, 'fg:cyan', matcherName)}: "endPattern" must be a string or a RegExp.`);
    let escapePattern = resolveArgument('escapePattern');
    let abortPatterns = this.resolveValue(context, this.abortPatterns, { wantPrimitiveValue: true });
    ...
    for (; offset < end;) {
      ...
      if (abortPatterns && abortPatterns.length > 0) {
        ...
        if (abortPatternIndex >= 0) {
          this.debugLog(context, `${this.debugColor(context, 'fg:cyan', matcherName)}: Match failed because abort pattern encountered at offset ${offset} (pattern: ${this.debugValue(context, abortPatterns[abortPatternIndex])}).`);
      ...
      if (len > 0) {
        ...
        if (offset > end) {
          this.debugLog(context, `${this.debugColor(context, 'fg:cyan', matcherName)}: Match failed because its out of bounds.`);
          return this.failResult(context);
        }
...
export function Sequence(name, startPattern, endPattern, escapePattern, _abortPatterns) {
