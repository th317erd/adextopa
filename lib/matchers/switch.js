...
export const SwitchMatcher = Utils.makeKeysNonEnumerable(class SwitchMatcher extends ProcessMatcher {
  ...
  static FailMatcherResult = Utils.makeKeysNonEnumerable(class SwitchFailMatcherResult extends FailureMatcherResult {
    async process(matcherScope, _result) {
      let {
        matcher,
      } = matcherScope;

      return await matcher.nextMatcher.call(matcher, matcherScope);
    }
  });
