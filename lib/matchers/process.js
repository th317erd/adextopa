...
export const ProcessMatcher = Utils.makeKeysNonEnumerable(class ProcessMatcher extends Matcher {
  ...
  updateParserOffset(matcherScope, offset) {
    ...
    return this.setParserRange(matcherScope, new SourceRange({ start: offset, end: matcherRange.end }));
  }
  ...
  async handleMatcherResult(matcherScope, matcherResult, onProcessEnded) {
    ...
    context.debugLog(`${context.debugColor('fg:cyan', matcherName)}: ${typeName} moving to next matcher...`);
  ...
  async processMatcher(matcherScope, onProcessEnded) {
    ...
    if (typeof resolvedMatcher === 'function')
      resolvedMatcher = resolvedMatcher();
  ...
  createMatcherScope(context) {
    ...
    let range         = new SourceRange({ start: matcherScope.startOffset, end: matcherScope.startOffset });
    let processToken  = this.processToken = new Token({
      parent:         null,
...
export const Loop = Matcher.createMatcherMethod((_, ...matchers) => {
