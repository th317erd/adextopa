...
export const MessageMatcher = Utils.makeKeysNonEnumerable(class MessageMatcher extends Matcher {
  ...
  async exec(matcherScope) {
    ...
    let range = new SourceRange(startOffset, startOffset);
    ...
    if (token) {
      ...
      result.parserOffset(token.matchedRange.end);
...
export function Message(message, recoveryHelper) {
