...
const SAFE_ARRAY_ACCESS       = (/^(\d+|length|size)$/);
...
export function fetch(path, defaultValue) {
  ...
  let value;
  if (isType(this, 'String') && key === 'size')
    value = this.length;
  if (this !== global && isPlainObject(this) && Object.prototype.hasOwnProperty.call(this, key))
    ...
    value = this[(key === 'size') ? 'length' : key];
...
export function debugWatch(context, propertyName, read, write) {
  let currentValue = context[propertyName];
  ...
  Object.defineProperty(context, propertyName, {
    ...
    get:          () => {
      if (typeof read === 'function')
        return read(currentValue, context, propertyName);

      return currentValue;
    },
    set:          (newValue) => {
      if (typeof write === 'function')
        currentValue = write(newValue, currentValue, context, propertyName);
      else
