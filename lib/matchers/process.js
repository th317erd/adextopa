...
export const ProcessMatcher = Utils.makeKeysNonEnumerable(class ProcessMatcher extends Matcher {
  ...
  createMatcherScope(context) {
    ...
    let startIndex    = parseFloat(context.resolveValue(this.startIndex, { defaultValue: 0 }));
    let endIndex      = parseFloat(context.resolveValue(this.endIndex, { defaultValue: Infinity }));
    let step          = parseFloat(context.resolveValue(this.step, { defaultValue: 1 }));
