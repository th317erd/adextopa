...
export function Iterate(/* name?, startIndex?, endIndex?, ...matchers */) {
  let args        = Array.from(arguments);
  let argIndex    = 0;
  let name        = (typeof args[argIndex] === 'string' || Matcher.isVirtual(args[argIndex])) ? args[argIndex++] : undefined;
  let startIndex  = (typeof args[argIndex] === 'number' || Matcher.isVirtual(args[argIndex])) ? args[argIndex++] : undefined;
  let endIndex    = (typeof args[argIndex] === 'number' || Matcher.isVirtual(args[argIndex])) ? args[argIndex++] : undefined;
  let step        = (typeof args[argIndex] === 'number' || Matcher.isVirtual(args[argIndex])) ? args[argIndex++] : undefined;
