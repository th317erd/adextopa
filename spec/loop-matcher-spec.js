...
describe("$LOOP", function() {
  describe("$LOOP Matcher", function() {
    ...
    it("should be able to work with min", function() {
      ...
      var program   = $PROGRAM($MATCHES(/\w+/), $OPTIONAL($MATCHES(/\s+/)));
      var matcher1  = $LOOP(program, { typeName: 'TestType', min: 45 });
      var matcher2  = $LOOP(program, { typeName: 'TestType', min: 5 });

      var result1 = matcher1.exec(parser);
      var result2 = matcher2.exec(parser);
    ...
    it("should be able to work with max", function() {
      var parser  = new Parser('a b c d e f g h i j k l m n o p q r s t u v w x y z');
      var program = $PROGRAM($MATCHES(/\w+/), $OPTIONAL($MATCHES(/\s+/)));
      var matcher = $LOOP(program, { typeName: 'TestType', max: 8 });
      var result  = matcher.exec(parser);
    ...
    it("should be able to use a finalizer callback", function() {
      var parser  = new Parser('testing token matching');
      var program = $PROGRAM($MATCHES(/\w+/, 'Word'), $OPTIONAL($MATCHES(/\s+/, 'WhiteSpace')));
      var matcher = $LOOP(program, ({ token }) => {
        return token.clone({
          typeName: 'Words',
