...
export class MessageMatcher extends Matcher {
  static name = 'Message';
  ...
  async exec(context) {
    ...
    if (result && result.value)
      this.debugLog(context, `${this.debugColor(context, 'fg:cyan', matcherName)}: Match success ${this.debugColor(context, 'bg:cyan')}@${range.start}-${range.end}${this.debugColor(context, 'bg:black')}: Resulting error "${message}" turned into an Message token.`);
