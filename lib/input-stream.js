...
export const InputStream = Utils.makeKeysNonEnumerable(class InputStream extends TypeBase {
  ...
  compare(range, pattern, ensureRangeBounds) {
    if (Utils.isType(pattern, RegExp)) {
      ...
      return Utils.COMPARE_SUCCESS; // Success
    } else {
      ...
      if (String.prototype.startsWith)
        return (this.toString().startsWith(pattern, range.start)) ? Utils.COMPARE_SUCCESS : Utils.COMPARE_FAILURE;

      return (this.indexOf(pattern, range.start) === range.start) ? Utils.COMPARE_SUCCESS : Utils.COMPARE_FAILURE;
    }
  }
