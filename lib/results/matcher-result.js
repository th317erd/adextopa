...
export const MatcherResult = Utils.makeKeysNonEnumerable(class MatcherResult extends TypeBase {
  ...
  toObject() {
    ...
    for (let [ key, value ] of this.properties) {
      if (Object.prototype.hasOwnProperty.call(PROPERTY_NAME_MAP, key))
        obj[PROPERTY_NAME_MAP[key]] = value;
  ...
  valueOf() {
    let value = this.get(VALUE, this.get(TOKEN));
    return (value && typeof value.valueOf === 'function') ? value.valueOf() : value;
  }

  dynamicProperties() {
    return {
      [Utils.TO_PRIMITIVE_SYMBOL]:  () => this.resolveValue(),
    };
  }
