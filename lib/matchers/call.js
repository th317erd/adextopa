...
export class CallMatcher extends Matcher {
  ...
  async exec(context) {
    ...
    if (!keyName)
      return this.panicResult(context, 'Call: Specified key to fetch from scope is empty.');

    let scope = context.scope;
    let matcher = this.resolveValue(context, scope.get(keyName));

    if (!(matcher instanceof Matcher))
      return this.panicResult(context, 'Call: Resulting key value is not a matcher.');

    matcher = matcher.clone();
    if (this.hasCustomName)
      matcher.name = this.resolveValue(context, this.name, { wantPrimitiveValue: true });
