...
export const SequenceMatcher = Utils.makeKeysNonEnumerable(class SequenceMatcher extends Matcher {
  ...
  exec(context) {
    const resolveArgument = (name, error) => {
      let value = context.resolveValue(this[name], { wantPrimitiveValue: true });
    ...
    let abortPatterns = context.resolveValue(this.abortPatterns, { wantPrimitiveValue: true });
    ...
    for (; offset < end;) {
      ...
      if (len > 0) {
        ...
        context.debugLog(`${context.debugColor('fg:cyan', matcherName)}: Match success ${context.debugColor('bg:cyan')}@${context.range.start}-${offset}${context.debugColor('bg:black')}: [${context.debugValue(value)}]`);
