...
export const EqualsMatcher = Utils.makeKeysNonEnumerable(class EqualsMatcher extends Matcher {
  ...
  createMatcherScope(context) {
    ...
    return {
      ...matcherScope,
      pattern: context.resolveValueToString(this.pattern),
    };
  }
  ...
  exec(matcherScope) {
    ...
    let range = new SourceRange({ start: startOffset, end: startOffset + pattern.length });
...
export const Equals = Matcher.createMatcherMethod((_, pattern) => {
