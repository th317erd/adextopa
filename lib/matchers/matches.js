...
export const MatchesMatcher = Utils.makeKeysNonEnumerable(class MatchesMatcher extends Matcher {
  ...
  constructor(_options) {
    ...
    let suppressIndices = options.suppressIndices;
    if (suppressIndices == null)
      suppressIndices = options._suppressIndices;

    suppressIndices = (suppressIndices != null) ? !!suppressIndices : false;
  ...
  exec(matcherScope) {
    ...
    let matchedRange = new SourceRange({ start: startOffset, end: startOffset + compareResult[0].length });
    ...
    if (indices && indices.length > 1) {
      ...
      for (let i = 1, il = indices.length; i < il; i++) {
        ...
        capturedRange.expandTo(new SourceRange({ start: indexRange[0], end: indexRange[1] }));
      }
    } else {
      capturedRange = new SourceRange({ start: startOffset, end: startOffset + compareResult[0].length });
    }
...
export const Matches = Matcher.createMatcherMethod((_, pattern) => {
