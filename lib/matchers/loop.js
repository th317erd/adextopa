...
export class LoopMatcher extends Matcher {
  ...
  updateLoopTokenRanges({ context, loopToken }, token) {
    if (loopToken.children.length === 0)
    ...
    if (token.matchedRange.end > context.range.start)
  ...
  processMatcherResultPayload(scope, _matcherResult) {
    let matcherResult = _matcherResult;
    if (matcherResult && matcherResult.payload)
      this.processMatcherResult(scope, matcherResult.payload);
  ...
  [MatcherResult.RESULT_EMPTY](scope) {
    let {
      context,
      matcherName,
      typeName,
  ...
  [MatcherResult.RESULT_PANIC](scope, matcherResult) {
    let {
      context,
      matcherName,
      typeName,
    ...
    return this.createProcessResponse(
      scope,
      PROCESS_CODE_BREAK,
      this.processMatcherResultPayload(scope, matcherResult),
    );
  }

  [MatcherResult.RESULT_FAIL](scope, matcherResult) {
    let {
      context,
      matcherName,
      typeName,
  ...
  [MatcherResult.RESULT_HALT](scope, matcherResult) {
    let {
      context,
      matcherName,
      typeName,
  ...
  [MatcherResult.RESULT_BREAK](scope, matcherResult) {
    let {
      context,
      matcherName,
      typeName,
    ...
    let result = this.processMatcherResultPayload(scope, matcherResult);
  ...
  [MatcherResult.RESULT_CONTINUE](scope, matcherResult) {
    let {
      context,
      matcherName,
      typeName,
  ...
  [MatcherResult.RESULT_TOKEN](scope, matcherResult) {
    let {
      context,
      loopToken,
      matcherName,
    ...
    this.updateLoopTokenRanges(scope, token);
  ...
  [MatcherResult.RESULT_PROXY_CHILDREN](scope, matcherResult) {
    let {
      context,
      matcherName,
    } = scope;
    ...
    for (let child of token.children.values())
      this[MatcherResult.RESULT_TOKEN](scope, new MatcherResult(MatcherResult.RESULT_TOKEN, child));
  ...
  [MatcherResult.RESULT_SKIP](scope, matcherResult) {
    let {
      context,
      loopToken,
      matcherName,
  ...
  processMatcherResult(scope, result) {
    return this[result.type](scope, result);
  }
  ...
  finalizeToken(scope) {
    ...
    if (loopToken.matchedRange.end === start)
      return;
  ...
  async executeMatcher(scope, matcher) {
    ...
    if (!(matcher instanceof Matcher)) {
      console.log('BAD MATCHER!', matcher);
    ...
    return this.processMatcherResult(scope, matcherResult || EMPTY_RESULT);
  }
  ...
  async executeMatchers(scope) {
    ...
    for (let matcher of matchers) {
      let result = await this.executeMatcher(scope, this.resolveValue(context, matcher));
      ...
      if (loopToken.offset >= end) {
        ...
        return this.createProcessResponse(scope, PROCESS_CODE_BREAK, result.result);
      }
  ...
  async exec(context) {
    ...
    context.assignToScope({ [matcherName]: loopToken });

    let startIndex  = parseFloat(this.resolveValue(context, this.startIndex, 0));
    let endIndex    = parseFloat(this.resolveValue(context, this.endIndex, Infinity));
    let step        = parseFloat(this.resolveValue(context, this.step, 1));
...
export function Loop(/* name?, ...matchers */) {
  ...
  if (typeof name === 'string' || (name instanceof Matcher && name._isFetchMatcher === true))
    args = args.slice(1);
  else
    name = undefined;
...
export function Iterate(/* name?, startIndex?, endIndex?, ...matchers */) {
  let args        = Array.from(arguments);
  let argIndex    = 0;
  let name        = (typeof args[argIndex] === 'string' || (args[argIndex] instanceof Matcher && args[argIndex]._isFetchMatcher === true)) ? args[argIndex++] : undefined;
  let startIndex  = (typeof args[argIndex] === 'number' || (args[argIndex] instanceof Matcher && args[argIndex]._isFetchMatcher === true)) ? args[argIndex++] : undefined;
  let endIndex    = (typeof args[argIndex] === 'number' || (args[argIndex] instanceof Matcher && args[argIndex]._isFetchMatcher === true)) ? args[argIndex++] : undefined;
  let step        = (typeof args[argIndex] === 'number' || (args[argIndex] instanceof Matcher && args[argIndex]._isFetchMatcher === true)) ? args[argIndex++] : undefined;
