...
export class LoopMatcher extends Matcher {
  ...
  finalizeToken(scope) {
    ...
    this.debugLog(context, `${this.debugColor(context, 'fg:cyan', matcherName)}: ${typeName} success ${this.debugColor(context, 'bg:cyan')}@${processToken.matchedRange.start}-${processToken.matchedRange.end}${this.debugColor(context, 'bg:black')}: [${this.debugValue(context, processToken.matchedValue)}]`, processToken);
