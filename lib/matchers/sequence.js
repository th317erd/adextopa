...
export const SequenceMatcher = Utils.makeKeysNonEnumerable(class SequenceMatcher extends Matcher {
  ...
  exec(matcherScope) {
    ...
    let range = new SourceRange(startOffset, endOffset);
    ...
    for (; range.start < range.end;) {
      ...
      if (compareLength > 0) {
        ...
        return context.tokenResult({
          ...
          matchedRange:   new SourceRange(context.parserRange.start, range.start),
          attributes:     {
            name:   matcherName,
            value:  value,
          },
...
export const Sequence = Matcher.createMatcherMethod((startPattern, endPattern, escapePattern, _abortPatterns) => {
