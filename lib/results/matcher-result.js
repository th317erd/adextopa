...
export const MatcherResult = Utils.makeKeysNonEnumerable(class MatcherResult extends TypeBase {
  ...
  *keys() {
    let keys = Array.from(this.properties.keys());
    if (this.parent)
      keys = Array.from((new Set(Array.from(this.parent.keys()).concat(keys))).values());
  ...
  *values() {
  ...
  setBreak(target) {
    return this.set(BREAK, target);
  }

  setContinue(target) {
    return this.set(CONTINUE, target);
  }
