...
export const ParserContext = Utils.makeKeysNonEnumerable(class ParserContext extends Scope {
  ...
  resolveValue(value, options) {
    if (value && typeof value[Utils.VIRTUAL_RESOLVER] === 'function') {
      let result = this.resolveValue(value[Utils.VIRTUAL_RESOLVER](this, options), options);
      if (result == null)
        return (options) ? options.defaultValue : value;
  ...
  async exec(_matcher) {
    ...
    let matcherName = this.resolveValueToString(matcher.name());
