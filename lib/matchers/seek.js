...
export class SeekMatcher extends Matcher {
  ...
  async exec(context) {
    ...
    let providedRange = this.resolveValue(context, this.range, { wantPrimitiveValue: false });
    let range         = this.getAbsoluteRange(context, providedRange);
    let matcher       = this.resolveValue(context, this.matcher, { wantPrimitiveValue: false });

    if (!matcher && (providedRange instanceof Matcher))
      matcher = this.resolveValue(context, providedRange, { wantPrimitiveValue: false });

    if (!(matcher instanceof Matcher)) {
      this.debugLog(context, `${this.debugColor(context, 'fg:cyan', matcherName)}: Requesting parent loop change parser range to: ${range}`);
      return this.seekResult(context, range);
    }

    this.debugLog(context, `${this.debugColor(context, 'fg:cyan', matcherName)}: Seeking to range ${range} and running provided matcher.`);
    let newContext = this.buildNewContext(context, false);
    newContext.range.setTo(range);

    let result = await matcher.run(newContext);

    this.debugLog(context, `${this.debugColor(context, 'fg:cyan', matcherName)}: Resetting range back to original ${context.range} after running provided matcher.`);
