...
export const ParserContext = Utils.makeKeysNonEnumerable(class ParserContext extends Scope {
  ...
  resolveValue(value, options) {
    if (value && typeof value[Utils.VIRTUAL_RESOLVER] === 'function')
      return this.resolveValue(value[Utils.VIRTUAL_RESOLVER](options), options);
  ...
  result(parent) {
    return new MatcherResult(parent);
  }
  ...
  breakResult(target, parent) {
    return this.result(parent)
      .break((target && Utils.isType(target, 'String')) ? target : true);
  }

  continueResult(target, parent) {
    return this.result(parent)
      .continue((target && Utils.isType(target, 'String')) ? target : true);
  }

  tokenResult(_properties, parent) {
    let properties = _properties || {};
    let token = (Utils.isType(properties, Token)) ? properties : new Token(
      this,
      properties.parent,
      properties,
    );
    ...
    return this.result(parent)
      .token(token)
      .capturedRange(token.capturedRange)
      .matchedRange(token.matchedRange)
      .parserOffset(token.matchedRange.end);
  }
  ...
  async exec(_matcher) {
    ...
    let matcherName = ('' + this.resolvePrimitive(matcher.name));
